		...
		jQuery('.speedycache-sortable').sortable(
			{
				...
				create : speedycache_sort_on_load
			}
		);
...
//Sorts the Position of Pages in the UI
function speedycache_sort_on_load() {
	if(!speedycache_ajax.preload_order) {
		return;
	}
	
	var preload_order = speedycache_ajax.preload_order.split(','),
	sortable_arr = jQuery('.speedycache-sortable .ui-sortable-handle');
	
	if(sortable_arr.length != preload_order.length) {
		return;
	}
	
	for(var i = 0; i < preload_order.length; i++) {
		if(jQuery(sortable_arr[i]).data('type') == preload_order[i]) {
			continue;
		}
		
		sortable_arr.each(function() {
			if(jQuery(this).data('type') != preload_order[i]) {
				return;
			}
			
			jQuery(this).before(sortable_arr[i]);
		});
	}
}
